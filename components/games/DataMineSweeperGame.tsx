...
const DataMineSweeperGame: React.FC<GameProps> = ({ onWin, onBet, currentBalance }) => {
  ...
  const floodFill = (r: number, c: number, currentGrid: MineSweeperCell[][]): MineSweeperCell[][] => {
    ...
    let revealCountDelta = 0; // Count newly revealed cells in this flood fill operation
    let prizeDelta = 0;
  ...
  const handleCellClick = (r: number, c: number) => {
    ...
    if (cell.adjacentMines === 0 && (!cell.prize || cell.prize === 0) ) { 
        const oldRevealedCount = revealedSafeCellsCount;
        const oldPrizes = currentRoundWinnings;
        newGrid = floodFill(r, c, grid); // floodFill updates states directly
        newlyRevealedCount = revealedSafeCellsCount - oldRevealedCount; // This will be incorrect if floodFill is async with setState
                                                                    // floodFill now updates states itself.
    } else { 
        newGrid[r][c] = { ...cell, isRevealed: true };
    ...
    // Check for win condition after state updates from floodFill or single click have settled
    // Need to use a useEffect or check based on the counts from the current operation
    // For simplicity, we'll assume revealedSafeCellsCount is up-to-date after the operation
    // Let's rely on the state update being processed.

    // Post-click message update logic will be handled in a useEffect watching relevant states
    if (newlyFoundPrize > 0) {
        // Message will be updated by useEffect watching currentRoundWinnings
    } else {
        setMessage(null); 
    }
  };
  ...
  useEffect(() => {
    if (gameState === 'playing') {
        ...
        } else if (currentRoundWinnings > 0 && !message?.startsWith("Найден приз") && !message?.startsWith("Все чисто")) {
             // This condition is tricky; only update if it's a new prize discovery not covered by loss/win
        }
    }
  ...
  const getCellContent = (cell: MineSweeperCell) => {
    ...
    if (cell.prize && cell.prize > 0) return `${cell.prize}${CURRENCY_SYMBOL}`;
    if (cell.adjacentMines > 0) return cell.adjacentMines.toString();
    return '';
  };

  const getCellClasses = (cell: MineSweeperCell) => {
    let classes = "aspect-square w-full h-full flex items-center justify-center text-xs sm:text-sm font-bold rounded transition-all duration-100 ease-in-out border ";
    
    if (cell.isFlagged) {
      classes += "bg-yellow-400 dark:bg-yellow-600 border-yellow-500 dark:border-yellow-700";
    } else if (cell.isRevealed) {
      if (cell.isMine) {
        classes += "bg-red-500 dark:bg-red-700 border-red-600 dark:border-red-800 animate-pulse text-white";
    ...
    <div className="flex flex-col items-center space-y-4 p-2 sm:p-4">
      {isLoading && (gameState === 'idle' || !grid.length) && (
          <div className="flex flex-col items-center space-y-2 h-64 justify-center">
            <Spinner />
            <p className="text-sm text-scrazino-gray-400">Подготовка поля...</p>
          </div>
      )}
      
      {grid.length > 0 && (
        <>
        <div className="flex justify-between w-full max-w-sm mb-2 text-sm text-scrazino-gray-700 dark:text-scrazino-gray-200">
            <span>Мины: {MINESWEEPER_NUM_MINES - flagsPlaced}</span>
            <span>Собрано: {currentRoundWinnings.toLocaleString()}{CURRENCY_SYMBOL}</span>
        </div>
        <div
          className={`grid gap-0.5 sm:gap-1 bg-scrazino-gray-600 p-1 sm:p-2 rounded-md shadow-lg`}
          style={{ gridTemplateColumns: `repeat(${MINESWEEPER_GRID_SIZE}, minmax(0, 1fr))` }}
      ...
      {message && <p className={`text-center font-semibold mt-2 ${gameState === 'won' || gameState === 'cashed_out' ? 'text-green-400' : (gameState === 'lost' ? 'text-red-400' : 'text-scrazino-yellow')}`}>{message}</p>}
      
      <div className="w-full max-w-md space-y-3 mt-4">
        {gameState === 'idle' || gameState === 'lost' || gameState === 'won' || gameState === 'cashed_out' ? (
          <>
            <div className="flex items-center space-x-2">
              <label htmlFor="betAmountMines" className="text-sm font-medium text-scrazino-gray-700 dark:text-scrazino-gray-300 whitespace-nowrap">Сумма ставки:</label>
              <input
                type="number"
                id="betAmountMines"
                value={betAmount}
            ...
            <div className="grid grid-cols-5 gap-2">
              {quickBetAmounts.map(qb => (
                <Button key={qb} variant="secondary" size="sm" onClick={() => { setBetAmount(qb); if (error) setError(''); }} disabled={(isLoading && gameState !== 'idle') || qb > currentBalance}>
                  {qb}
                </Button>
              ))}
            </div>
            {error && <p className="text-sm text-red-500 dark:text-red-400 text-center">{error}</p>}
            <Button onClick={handleStartGame} size="lg" className="w-full !text-xl" disabled={isLoading && gameState !== 'idle'}>
              {gameState === 'idle' ? 'Сделать ставку и Начать' : 'Играть снова со ставкой'}
            </Button>
          </>
        ) : ( 
          <>
            <Button onClick={handleCashOut} variant="primary" size="lg" className="w-full !text-xl bg-green-600 hover:bg-green-700" disabled={currentRoundWinnings === 0 || isLoading}>
