...
const CosmicCrashGame: React.FC<GameProps> = ({ onWin, onBet, currentBalance }) => {
  ...
  const renderMultiplierGraph = () => {
    ...
    return (
      <div className="w-full h-56 bg-scrazino-gray-700 dark:bg-scrazino-gray-900 rounded-lg p-4 relative overflow-hidden shadow-inner">
        ...
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-4xl sm:text-5xl font-bold ${color} transition-colors duration-300`}
  ...
  return (
    <div className="flex flex-col items-center space-y-6 p-4">
      ...
      {message && (
        <p className={`text-lg font-semibold ${gameState === 'cashed_out' ? 'text-green-400' : (gameState === 'crashed' ? 'text-red-400' : 'text-scrazino-gray-300')}`}>
          {message}
        </p>
      )}
      {(gameState === 'betting' || (gameState === 'running' && !currentMultiplier)) && <Spinner />}
        ...
        {gameState === 'idle' || gameState === 'crashed' || gameState === 'cashed_out' ? (
          <>
            <div className="flex items-center space-x-2">
              <label htmlFor="betAmountCrash" className="text-sm font-medium text-scrazino-gray-300 whitespace-nowrap">Сумма ставки:</label>
              <input
                type="number"
                id="betAmountCrash"
                value={betAmount}
            ...
            <Button onClick={gameState === 'idle' ? handleBet : resetGame} size="lg" className="w-full !text-xl">
          ...
          <Button 
            ...
            variant="primary" 
            size="lg" 
            className="w-full !text-xl bg-green-500 hover:bg-green-600"
          >
