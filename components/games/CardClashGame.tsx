...
const CardDisplay: React.FC<{ card: PlayingCard | null; hidden?: boolean }> = ({ card, hidden }) => {
  const suitColor = card && (card.suit === '♥' || card.suit === '♦') ? 'text-red-500' : 'text-scrazino-gray-800 dark:text-scrazino-gray-200';
  
  if (hidden || !card) {
    return (
      <div className="w-24 h-36 sm:w-28 sm:h-40 bg-scrazino-gray-300 dark:bg-scrazino-gray-700 rounded-lg shadow-xl border-2 border-scrazino-gray-400 dark:border-scrazino-gray-600 flex items-center justify-center">
        <span className="text-4xl text-scrazino-gray-500 dark:text-scrazino-gray-400">?</span>
      </div>
    );
  }

  return (
    <div className={`w-24 h-36 sm:w-28 sm:h-40 bg-white dark:bg-scrazino-gray-100 rounded-lg shadow-xl border-2 border-scrazino-gray-300 dark:border-scrazino-gray-500 p-2 flex flex-col justify-between items-center ${suitColor}`}>
      <div className="self-start text-xl sm:text-2xl font-bold">{card.rank}</div>
      <div className="text-3xl sm:text-4xl">{card.suit}</div>
      <div className="self-end text-xl sm:text-2xl font-bold transform rotate-180">{card.rank}</div>
    </div>
  );
};
    ...
    <div className="flex flex-col items-center space-y-4 p-4">
      <div className="flex space-x-4 mb-4">
        <div className="flex flex-col items-center">
            <p className="text-sm text-scrazino-gray-400 mb-1">Текущая карта</p>
            <CardDisplay card={currentCard} />
        </div>
        <div className="flex flex-col items-center">
             <p className="text-sm text-scrazino-gray-400 mb-1">Следующая карта</p>
            <CardDisplay card={nextCard} hidden={gameState !== 'revealed' && !nextCard && !isProcessing} />
        </div>
      </div>

      {message && <p className={`text-center font-semibold ${gameState === 'revealed' ? 'text-green-400' : (gameState === 'lost' ? 'text-red-400' : 'text-scrazino-yellow')}`}>{message}</p>}
        ...
        {gameState === 'idle' || gameState === 'lost' ? (
          <>
            <div className="flex items-center space-x-2">
              <label htmlFor="betAmountCards" className="text-sm font-medium text-scrazino-gray-300 whitespace-nowrap">Начальная ставка:</label>
              <input
                type="number"
                id="betAmountCards"
                value={betAmount}
                ...
                min="1"
                max={currentBalance}
                className="w-full px-3 py-2 border border-scrazino-gray-600 rounded-lg shadow-sm focus:ring-scrazino-yellow focus:border-scrazino-yellow bg-scrazino-gray-700 text-white"
              />
            </div>
            <div className="grid grid-cols-5 gap-2">
              {quickBetAmounts.map(qb => (
                <Button key={qb} variant="secondary" size="sm" onClick={() => { setBetAmount(qb); if(error) setError(''); }} disabled={qb > currentBalance}>
                    {qb}
                </Button>
              ))}
            </div>
            {error && <p className="text-sm text-red-400 text-center">{error}</p>}
            <Button onClick={handleStartRound} size="lg" className="w-full !text-xl" disabled={isProcessing}>
              {gameState === 'idle' ? 'Сделать ставку и Раздать' : 'Играть снова'}
            </Button>
          </>
        ) : (
          <>
            <p className="text-center text-scrazino-yellow">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button onClick={() => handleGuess('higher')} size="lg" variant="primary" className="!text-lg" disabled={isProcessing}>Больше</Button>
              <Button onClick={() => handleGuess('lower')} size="lg" variant="primary" className="!text-lg" disabled={isProcessing}>Меньше</Button>
            </div>
            {gameState === 'revealed' && currentStreakWinnings > 0 && (
                 <Button onClick={handleCashOut} size="lg" variant="secondary" className="w-full !text-xl bg-green-600 hover:bg-green-700" disabled={isProcessing}>
